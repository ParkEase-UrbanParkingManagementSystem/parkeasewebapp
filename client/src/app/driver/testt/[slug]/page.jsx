...
export default function ParkingLotDetails() {
  ...
  useEffect(() => {
    ...
    const fetchParkingLotDetails = async () => {
      try {
        ...
        const response = await fetch(`${process.env.NEXT_PUBLIC_API_KEY}/parkinglots/${slug}`, {
          method: 'GET',
          headers: {
  ...
  if (loading) return <div className="text-center text-gray-600 p-8">Loading...</div>;
  if (error) return <div className="text-center text-red-500 p-8">{error}</div>;
  if (!parkingLot) return <div className="text-center text-gray-600 p-8">No details found for this parking lot.</div>;
...
/>
          ...
          <div className="md:w-1/2 p-6">
            <h1 className="text-3xl font-bold text-gray-800 mb-4">{parkingLot?.lot?.name}</h1>
            <p className="text-gray-600 mb-6">
              {parkingLot?.lot?.addressno}, {parkingLot?.lot?.street1}, {parkingLot?.lot?.street2}, {parkingLot?.lot?.city}, {parkingLot?.lot?.district}
            </p>
            <div className="grid grid-cols-2 gap-4 mb-6">
              <div className="bg-white p-4 rounded-lg shadow">
                <h2 className="text-lg font-semibold mb-2">Bike Capacity</h2>
                <p className="text-2xl font-semibold text-green-600">
                  {parkingLot?.lot?.bike_capacity_available}/{parkingLot?.lot?.bike_capacity}
                </p>
              </div>
              <div className="bg-white p-4 rounded-lg shadow">
                <h2 className="text-lg font-semibold mb-2">Car Capacity</h2>
                <p className="text-2xl font-semibold text-green-600">
                  {parkingLot?.lot?.car_capacity_available}/{parkingLot?.lot?.car_capacity}
                </p>
              </div>
            </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                {parkingLot?.slotPrices?.map((price) => (
                  <div key={price.type_name} className="flex justify-between">
                    ...
                    <span className="font-semibold">Rs. {price.amount_per_vehicle}</span>
                  </div>
                ))}
              </div>
            </div>
                  ...
                  <div className="flex items-center mb-2">
                    <div className="w-12 h-12 rounded-full bg-gray-200 flex items-center justify-center overflow-hidden">
                      {review.profile_pic ? (
                        ...
                        <span className="text-gray-600">{review.driver_fname?.[0]}{review.driver_lname?.[0]}</span>
                      )}
                    </div>
                    <div className="ml-4">
                      <p className="font-semibold text-gray-800">{review.driver_fname} {review.driver_lname}</p>
