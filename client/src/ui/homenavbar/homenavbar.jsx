...
const pages = ["Home", "Contact Us", "Your Activity", "Your Vehicles"];
...
const Navbar = () => {
  ...
  const fetchNotifications = async () => {
    ...
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_KEY}/notifications/get-notifications`,
        {
          method: "GET",
          headers: {
  ...
  const markAsRead = async (id) => {
    ...
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_KEY}/notifications/mark-read`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
  ...
  const markAllAsRead = async () => {
    ...
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_KEY}/notifications/mark-read-all`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
        ...
        <Toolbar disableGutters>
          ...
          <Box sx={{ flexGrow: 0 }}>
            ...
            <Tooltip title="View More">
              <IconButton onClick={handleOpenUserMenu} sx={{ paddingLeft: 4 }}>
                <Avatar
                  sx={{ width: 40, height: 40, backgroundColor: "#ffb403", color: "black", fontWeight: "bold", }}
                >
                  {userDetails?.fname?.charAt(0)?.toUpperCase()}
                  {userDetails?.lname?.charAt(0)?.toUpperCase()}
                </Avatar>
              </IconButton>
            </Tooltip>
